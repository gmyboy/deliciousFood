...
public class HttpDownloader {
	...
	public boolean download() {
		...
		try {
			...
			log.info("分配线程。线程数量=" + threads + ",块总数=" + blocks + ",总字节数=" + fileLength + ",每块大小=" + blockSize + ",块/线程=" + blockPage);
			// 检查
			running = true;
			// 创建一个线程组,方便观察和调试
			...
			for (int i = 0; i < threads; i++) {
				...
				// 启动下载其他线程
				DownloadThread downloadThread = new DownloadThread(downloadGroup, i, begin, end);
		...
		log.info("下载完成，耗时:" + getTime((System.currentTimeMillis() - beginTime) / 1000));
		return ok;
	}
	...
	private void loadPrevious() throws Exception {
		...
		log.info("之前的文件大小应该是:" + blockSet.length * 8l * blockSize + ",一共有:" + blockSet.length + "块");
	}
	...
	private HttpURLConnection getHttpConnection(long pos) throws IOException {
		...
		if (useProxy) {
			log.debug("使用代理进行连接.ProxyServer=" + proxyServer + ",ProxyPort=" + proxyPort);
	...
	private String getDesc() {
		...
		return String.format("已下载/总大小=%s/%s(%s),速度:%s,耗时:%s,剩余大小:%d", getFileSize(downloadBytes), getFileSize(fileLength), getProgress(fileLength, downloadBytes),
				getFileSize(downloadBytes / ((System.currentTimeMillis() - beginTime) / 1000 + 1)), getTime((System.currentTimeMillis() - beginTime) / 1000), fileLength - downloadBytes);
	}
	...
	private class DownloadThread extends Thread {
		...
		public DownloadThread(ThreadGroup group, int id, int blockBegin, int blockEnd) throws Exception {
		...
		public void run() {
			...
			try {
				...
				log.info(getThreadName() + ":定位文件位置.Pos=" + 1l * blockBegin * blockSize);
				destFile.seek(1l * blockBegin * blockSize);
				log.info(getThreadName() + ":开始下载.[ " + blockBegin + " - " + blockEnd + "]");
				...
				while (blockBegin < blockEnd) {
					...
					if (BitUtil.getBit(blockSet, blockBegin)) {
						...
						while (counts < blockSize) {
							int read = inputStream.read(b, 0, blockSize - counts);
							if (read < 0)
								break;
							counts += read;
